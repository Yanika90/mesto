...
const formElementProfile = document.querySelector(
  '.popup__input-form_type_edit-profile'
...
const formElementCard = document.querySelector(
  '.popup__input-form_type_add-card'
)
...
const popupCloseButtonImageOpen = document.querySelector(
  '.popup__close-button_type_image-open'
)
...
const createCard = card => {
  const newCard = document
    ...
    .content.cloneNode(true) // содержимое template передается методом .content
  ...
  // обработчик открытия просмотра изображения cardImage.addEventListener('click', () => console.log('test'));
